...
- `test/windows-shell.sh <cranamp.exe> <out-dir> [ssh-host] [--console-only]` runs `test/windows-shell.ps1` on a signed-in Windows desktop over SSH: the executable must be a GUI program with the Cranamp icon and version resource, open no terminal, and give every window the icon in its title bar and taskbar. `--console-only` checks any Cranpose app: a GUI program that opens no console window while it starts.
