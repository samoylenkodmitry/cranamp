...
fn operation_schema() -> Value {
    json!({"type":"array","items":{"type":"object","required":["op"],"properties":{
        ...
        "color":{"type":"string","description":"#rrggbb or #rrggbbaa. BMP sprites are opaque: #ff00ff is ordinary magenta. transparent / alpha 0 erases the active paint layer; lower paint must supply an opaque background before export."},
        ...
        "rows":{"type":"array","items":{"type":"string"},"description":"One character per pixel. Missing palette characters SKIP and preserve existing paint. Every exported sprite cell needs an opaque background. Moving handles carry their complete rectangle; match the track background, never a fixed slice of an illustration."},
...
fn tools() -> Vec<Value> {
    vec![
        ...
        tool("studio_eq", "EQ workbench shared with the GUI. All eleven bands share 28 track frames and two handles; levels are not independent band pictures. Select a frame/handle/background to draw with the normal pen, copy one frame to all 28, or preview mixed positions. Artwork mode preserves source layers but exports a 163-row EQMAIN.BMP like the Bjork reference: slider, graph and preset graphics disappear while hit targets remain.", json!({"action":{"enum":["inspect","set_mode","edit_frame","edit_handle","edit_background","copy_frame_to_all","preview"]},"mode":{"enum":["controls","artwork"]},"frame":{"type":"integer","minimum":0,"maximum":27},"scope":{"enum":["current","all"]},"pressed":{"type":"boolean"},"pattern":{"enum":["flat","ramp","alternating"]}}), &[]),
        ...
        tool("studio_export", "Export a genuine classic WSZ with opaque 24-bit BMPs and explicit palettes. The same format gate protects GUI export, MCP export and Apply to player. Invalid or unfinished source cells refuse BEFORE writing. Warnings about composition/readability still require visual review in the GPU and external players. Magenta never becomes transparent.", json!({"path":{"type":"string","description":"Output .wsz path; the adjacent .cstudio preserves editable paint layers."}}), &["path"]),
    ].into_iter().map(|mut entry| {
        if entry["name"] == "studio_canvas" {
            entry["inputSchema"]["properties"]["drawer"]["enum"] = json!(super::model::DRAWERS);
        }
        if entry["name"] == "studio_draw" {
            entry["description"] = json!("One undoable native-pen transaction, up to 10000 operations, all or nothing. Reports source writes, clipped/unmapped/unsampled pixels, shared-cell overwrites and hidden ink. Classic BMP cells are opaque: magenta is ordinary ink and alpha only edits the paint layer. Review continuity, covering sources, flat drawable areas and the actual GPU. A successful write does not approve the composition.");
        }
        if entry["name"] == "studio_options" {
            entry["inputSchema"]["properties"]["classic_preview"] = json!({"type":"boolean","const":true,"description":"Deprecated compatibility field. Classic opaque BMP semantics are always enabled; false is rejected."});
        }
        if entry["name"] == "studio_draw" {
            entry["inputSchema"]["properties"]["require_continuity"] = json!({"type":"boolean","description":"On the joined canvas, refuse and roll back a draw whose final opaque ink is clipped, hidden/replaced by another sprite, conflicting in shared cells, or unmapped. states:all checks all four active/pressed composites. Magenta counts as opaque ink. The report includes native samples and covering sources. Preview reports without committing."});
        }
        entry
    }).collect()
}
...
pub fn call(name: &str, args: Value, shared: &SharedDocument) -> Result<Value> {
    ...
    let value = match name {
        ...
        "studio_eq" => doc.eq_workbench(&args, "MCP")?,
        "studio_new" => {
        ...
        "studio_canvas" | "studio_atlas" => {
            ...
            if name == "studio_atlas" {
                ...
                view["panel"] = json!(match args.get("surface").and_then(Value::as_str) {
                    Some("shade") => "shade",
                    Some("cursors") => "cursors",
                    _ => "canvas",
                });
            }
            if let Some(v) = view.as_object_mut() {
                v.remove("surface");
            }
        ...
        "studio_targets" => {
            if args.get("auto") == Some(&json!(true)) {
                ...
                // The rolled-up player is a surface of its own, not a window
                // of the joined skin, so it lists its own sprites while open.
                let (blank, all) = if doc.view.panel == "shade" {
                    (doc.undrawn_sprites_on("shade"), doc.layers())
                } else {
                    (doc.undrawn_sprites(), doc.skin_layers())
                };
                let total = all.len();
                let sprites = all
                    .into_iter()
        ...
        "studio_regions" => doc.window_regions(&args)?,
        "studio_font" => doc.classic_font(
            args["ink"].as_str().context("ink required")?,
            args["background"].as_str().context("background required")?,
        )?,
        "studio_options" => {
            if let Some(classic) = args.get("classic_preview") {
                anyhow::ensure!(classic.as_bool() == Some(true), "Classic BMP semantics are mandatory. Magenta sprite holes are no longer supported.");
            }
            ...
            json!({
                "classic_preview": doc.view.classic_preview,
                "preview_scope": "Classic opaque BMP rendering and REGION.TXT window cutouts are applied in the editor and GPU player. Shade masks are preserved for external players.",
                ...
                "visualizer_background": {
                    "transparent": false,
                    "note": "VISCOLOR slot 0 is an opaque RGB background. Magenta is an ordinary color."
                },
