    ...
    let value = match name {
        ...
        "studio_targets" => {
            if args.get("auto") == Some(&json!(true)) {
                ...
                // The rolled-up player and the footer are surfaces of their
                // own, not windows of the joined skin, so each lists its own
                // sprites while open.
                let (blank, all) = if matches!(doc.view.panel.as_str(), "shade" | "footer") {
                    (doc.undrawn_sprites_on(&doc.view.panel), doc.layers())
                } else {
